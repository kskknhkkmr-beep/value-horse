...
export async function GET(request: Request) {
  ...
  const inputs = horsesWithOdds.map((h) => {
    ...
    const DEFAULT = 65;
    // form/pedigree は常に数値。jockey は欠損を null として保持
    ...
    return { id: h.id, name: h.horse, formScore, pedigreeScore, jockeyScore, odds: h.odds! };
  });
  ...
  const toShape = (h: (typeof result.finalScores)[number]) => {
    ...
    return {
      ...
      features: f ? { form: f.formScore, pedigree: f.pedigreeScore, jockey: f.jockeyScore } : null,
    };
  };
